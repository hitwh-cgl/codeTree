...
public class MaximizeTheConfusionOfAnExam_2024 {
    public static void main(String[] args) {
        ...
        int i = solution.maxConsecutiveAnswers(answerKey, 1);
    ...
    class Solution {
        ...
        private int max(String answerKey, int k, char ch) {
            // TTFTTFTT 1 ch=T
            int left = 0, right = 0;
            int count = answerKey.charAt(0) == ch ? 1 : 0;
            int max = 1;
            while (right < answerKey.length()) {
                while (right + 1 < answerKey.length() && count <= k) {
                    right++;
                ...
                while (left < right && answerKey.charAt(left) != ch) {
                    left++;
                }
                if (left <= right && answerKey.charAt(left) == ch) {
                    left++;
                    count--;
                } else {
                    break;
