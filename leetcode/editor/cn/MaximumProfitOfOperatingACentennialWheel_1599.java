...
public class MaximumProfitOfOperatingACentennialWheel_1599 {
    ...
    class Solution {
        public int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
            ...
            int maxProfit = 0;
            int maxProfitTurn = 0;
            int leftCustomer = customers[0];
            int profit = 0;
            for (int i = 1; i < customers.length || leftCustomer > 0; i++) {
                int curTurnCust = Math.min(4, leftCustomer);
                leftCustomer -= curTurnCust;
                profit += -runningCost + curTurnCust * boardingCost;
                if (profit > maxProfit) {
                    maxProfit = profit;
                    maxProfitTurn = i;
                }
                if (i < customers.length) {
                    leftCustomer += customers[i];
                }
            }
            return maxProfit > 0 ? maxProfitTurn : -1;
        }
    }
