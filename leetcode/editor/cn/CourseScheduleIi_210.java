    ...
    class Solution {
        ...
        public int[] findOrder(int numCourses, int[][] prerequisites) {
            ...
            for (int i = 0; i < numCourses; i++) {
                ...
                visitingSet.add(i);
                if (!visit(i, preMap, visitingSet, finished)) {
                    return new int[]{};
                }
            }
            return order;
        ...
        private boolean visit(int course, Map<Integer, List<Integer>> preMap, Set<Integer> visitingSet,
                              boolean[] finished) {
            List<Integer> preList = preMap.get(course);
            if (preList != null) {
                for (int curPre : preList) {
                    if (finished[curPre]) {
                        continue;
                    }
                    if (!visitingSet.add(curPre)) {
                        return false;
                    }
                    if (visit(curPre, preMap, visitingSet, finished)) {
                        visitingSet.remove(curPre);
                    } else {
                        return false;
                    }
                }
